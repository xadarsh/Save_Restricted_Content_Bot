...
@aadi.on(events.NewMessage(incoming=True, func=lambda e: e.is_private))
async def clone(event):
    ...
    for li in li:
        ...
        try:
            ...
            await aadi.send_message(event.sender_id, f'Try again after {fw.value} seconds due to floodwait from telegram.')
            await edit.delete()
        except Exception as e:
            logging.info(e)
            await aadi.send_message(event.sender_id, f"An error occurred during cloning of `{link}`\n\n**Error:** {str(e)}")
            await edit.delete()
